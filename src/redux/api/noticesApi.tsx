...
export const addApi = async (Data: FormData) => {
  try {
    const response = await api.post('web/notices', Data, {
      headers: {
...
export const getApi = async (params = {}) => {
  try {
    const response = await api.get('web/notices', {
      params,
    });
    return response.data;
  } catch (error: any) {
...
export const updateApi = async (id: string, Data: FormData) => {
  try {
    const response = await api.post(`web/notices/${id}`, Data, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
      params: {
...
export const showApi = async (id: string) => {
  try {
    const response = await api.get(`web/notices/${id}`);
    return response.data;
  } catch (error: any) {
    throw error.response ? error.response.data : new Error('Error fetching data');
  }
...
export const deleteApi = async (id: string) => {
  try {
    const response = await api.delete(`web/notices/${id}`);
    return response.data;
  } catch (error: any) {
    throw error.response ? error.response.data : new Error('Error fetching data');
  }
