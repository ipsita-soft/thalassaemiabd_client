...
import { useCreateFinancialDonationMutation } from '@/api/financialDonationApi';
...
const Add: React.FC = () => {
  ...
  const [createFinancial] = useCreateFinancialDonationMutation();
  ...
  const handleSubmit = async (values: any, { setErrors, resetForm }: any) => {
    setLoading(true);
    try {
      await createFinancial(values).unwrap();
      setOpen(false);
      resetForm();
      Swal.fire({
        title: 'Success!',
