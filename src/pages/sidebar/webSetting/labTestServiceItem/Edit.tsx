...
import { useFetchLabTestServiceItemQuery, useUpdateLabTestServiceItemMutation } from '@/api/labTestServiceItemApi';
...
const Edit: React.FC<EditProps> = ({ Id, onClose }) => {
  ...
  const { data } = useFetchLabTestServiceItemQuery(Id);
  const [updateLabTestItem, { isLoading: isUpdating }] = useUpdateLabTestServiceItemMutation();

  useEffect(() => {
    if (data?.data) {
      const { name, outdoor_rate, indoor_rate, subsidy, kit_cost_id, sorting_index, status } = data.data;
      setFormData({
        name: name || '',
        outdoor_rate: outdoor_rate || 0,
        indoor_rate: indoor_rate || 0,
        subsidy: subsidy || 0,
        kit_cost_id: kit_cost_id || 0,
  ...
  const validationSchema = Yup.object().shape({
    ...
    kit_cost_id: Yup.number().nullable(),
        ...
        >
          {({ errors, touched }) => (
            <Form className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="grid gap-2">
                <Label htmlFor="kit_cost_id">Kit Cost Id</Label>
                <Field
                  as={Input}
                  type="number"
                  id="kit_cost_id"
                  name="kit_cost_id"
                  placeholder="Enter Kit Cost Id"
                  aria-label="Kit Cost Id"
                  className={errors.kit_cost_id && touched.kit_cost_id ? 'border-red-500' : ''}
                />
                <ErrorMessage name="kit_cost_id" component="div" className="text-red-500 text-sm" />
              </div>
