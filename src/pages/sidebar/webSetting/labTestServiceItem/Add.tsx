...
import { useCreateLabTestServiceItemMutation } from '@/api/labTestServiceItemApi';

const Add: React.FC = () => {
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [createLabTest] = useCreateLabTestServiceItemMutation();
  ...
  const validationSchema = Yup.object().shape({
    ...
    kit_cost_id: Yup.number()
    .nullable()
    .min(0, 'Kit Cost Id must be a positive number.'),
  ...
  const handleSubmit = async (values: any, { setErrors }: any) => {
    ...
    try {
      const response = await createLabTest(values).unwrap();
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        <DialogHeader>
          <DialogTitle>Add New Lab Test Service Item</DialogTitle>
        </DialogHeader>
        <Formik
          initialValues={{
            name: '',
              ...
              <div className="row">
                ...
                <div className="col-md-6 mb-4">
                  <Field
                    name="kit_cost_id"
                    type="number"
                    placeholder="Kit Cost Id"
                    className="form-control"
                  />
                  <ErrorMessage name="kit_cost_id" component="div" className="text-danger" />
                </div>
