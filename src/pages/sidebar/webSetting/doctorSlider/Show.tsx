...
const Show: React.FC<EditSliderProps> = ({ Id }) => {
  ...
  const ToShow = doctorSliders.find((doctorSliders) => doctorSliders.id.toString() === Id) as Events | undefined;
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold"> {ToShow.name}</DialogTitle>
        </DialogHeader>
        <div className="grid gap-4">
          <div className="grid gap-2">
            ...
            <div className="flex justify-center">
              {ToShow.image ? (
                <img src={ToShow.image} alt="Slider" className="w-full h-auto rounded-md shadow-sm" />
              ) : (
