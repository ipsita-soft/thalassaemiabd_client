...
import { get } from "@/redux/slices/gallerySlice";
...
export function GalleryPage() {
    ...
    const { galleries, meta, isLoading, isError, error } = useSelector(
    ...
    const columns = [
...

        {
            ...
            cell: ({ row }: { row: { original: Data } }) => (
                <div className="text-left">{row.original.type}</div>
            ),
        },
        {
            id: "image",
...

        {
            ...
            cell: ({ row }: { row: { original: Data } }) => (
                row.original.type === 'video' ? (
                    <a href={row.original.video_url} target="_blank" rel="noopener noreferrer" className="text-left">
                        {row.original.video_url}
                    </a>
                ) : (
                    <span>No Video URL (Type is Image )</span> // Optional text when it's an image
                )
            ),
        },
    ...
    const table = useReactTable({
        data: galleries as unknown as Data[],
        columns,
        state: {
            sorting,
            columnFilters,
